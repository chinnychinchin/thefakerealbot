...
const randIntGen = () => {
  ...
  return Math.floor(Math.random() * fortune.length)

}

const checkIfInArray = (numArray, randInt) => {

  return !!numArray.find(num => num == randInt) //returns true if the randInt is already in the array

}

const xNumOfCookies = (x) => {
  ...
  for (let num = 0; num < x; num++) {
    ...
    do {
      randInt = randIntGen()
    }while(checkIfInArray(numArray, randInt));

    numArray.push(randInt);

  }
  
  const cookieArray = numArray.map(num => {return fortune[num]})
  return cookieArray;

}

module.exports = {xNumOfCookies}